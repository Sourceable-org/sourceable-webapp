...
const convertPrecision = (precision?: string): 'exact' | '5mile' | '10mile' | '20mile' | undefined => {
  if (precision === '5mi') return '5mile';
  if (precision === '10mi') return '10mile';
  if (precision === '20mi') return '20mile';
...
const VerifyScreen = () => {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center px-4 py-8">
      {/* Branding */}
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-center mb-8 space-x-3">
        <CheckBadgeIcon className="w-8 h-8 text-blue-600" />
        <span className="font-bold text-2xl tracking-tight text-gray-900">Sourceable</span>
      </motion.div>

      {/* Headline & Subheadline */}
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-center mb-6">
        <h1 className="text-5xl font-extrabold tracking-tight mb-2 text-blue-600">Simpler Proof</h1>
        <p className="text-lg text-gray-500">Capture and verify photos or videos in seconds.</p>
      </motion.div>

      {/* Upload/Action Button (if needed) */}
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="mb-8 w-full max-w-md">
        <button onClick={handleNewCapture} className="w-full py-3 rounded-2xl bg-blue-500 text-white text-lg font-semibold shadow-lg hover:bg-blue-600 transition">Upload</button>
      </motion.div>
      ...
      {metadata?.gps_lat && metadata?.gps_lng && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="w-full max-w-2xl bg-white rounded-2xl shadow p-6 mb-8">
      ...
      {/* Info Sections (Timestamp + GPS, Unique verification link, etc.) */}
      <div className="w-full max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center">
          ...
          <span className="text-blue-700 text-center break-all underline">{window.location.host}/verify/{id}</span>
        </div>
      </div>

      {/* QR Code Section */}
      {qrCodeUrl && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-white rounded-2xl shadow p-6 flex flex-col items-center mb-8">
          <span className="font-bold text-xl mb-2 text-blue-600">Scan to Verify</span>
      ...
      <div className="w-full max-w-2xl flex flex-col md:flex-row gap-4 mb-8">
        <button onClick={handleCopyLink} className="flex-1 py-3 rounded-xl bg-blue-100 text-blue-700 font-semibold shadow hover:bg-blue-200 transition">Copy Link</button>
        <button onClick={handleDownload} className="flex-1 py-3 rounded-xl bg-blue-500 text-white font-semibold shadow hover:bg-blue-600 transition">Download Media</button>
      </div>

      {/* Share Options (optional, can be styled further) */}
